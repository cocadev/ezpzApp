...
import {
  View, Text, StyleSheet, Dimensions,Platform,DatePickerAndroid,
  Image, TouchableOpacity, FlatList, TextInput, AsyncStorage, Alert, ActivityIndicator, Linking
} from 'react-native';
...
// const HelpTexts = (props) => {
//   return (
//     props.helptexts.map((val, idx)=> {
//       let textId = textId+'-'+idx;
//       return (
...
//                     value={props.helptexts[idx].name}
//                     ref={(ref) => this.props.setInputRef(ref, textId)}
//                     onChangeText={(value) => this.props.handleChangeText(value, textId)}
//           />
//         </View>)}
//     ))
//   }
...
export class ReviewOrder extends Component {
  constructor(props) {
    ...
    this.inputs = [];
    this.state = {
      couponCode: '',helptexts: [{name:""}],
      productQuantityVal: 1, userData: {},loading:false,itemsList:[],modList:[],
      taxRate: 6.35, loader: false,restData:null,infoType:[],
      isShowItemModal: false, modalData: [], cardlength: -1,
      cardList: [], cartempty: false, promptVisible: false, note: '', tableNumber: '', time: '',
      isTimeVisible: false, currentDate: new Date(), yes: true, settings: [], day: '',
      tips: [
        { price: 10 },
        { price: 15 },
        { price: 20 }
      ], selectedTip: -1, tipPrice: 0, tipType: false, tipModal: false, minuteInterval: 0
    };
  }
  ...
  componentWillMount = async () => {
    ...
    console.log(JSON.parse(await AsyncStorage.getItem("restaurant")));
    let restInfo = JSON.parse(await AsyncStorage.getItem("restaurant"));

    if(restInfo){
      
      if( restInfo.appSettings && restInfo.appSettings.orderTypeOptions){
        for(let item of restInfo.appSettings.orderTypeOptions){
           item.isChecked = false;
        }
      }
    }

    this.setState({ userData: JSON.parse(await AsyncStorage.getItem("UserData")),restData: restInfo}, () => {
      let d = new Date();
      this.setState({ day: moment(d).format('ddd').toLowerCase() })
      this.getcardListOnLoad();
      this.getnumbers();
    })
  }
  getcardListOnLoad=async()=> {
    ...
    this.setState({ cardList: await API.get("list", "/api/cart/Info?userId=" + this.state.userData.id, {}), loading: false,cardlength: 0 }, async() => {
        ...
        if (this.state.cardList.length == 0) {
          ...
          let allRest = JSON.parse(await AsyncStorage.getItem('allRest'));
          let restaurant = allRest.filter((item)=>{ return item.id == this.state.cardList[0].restaurantId});
          if(restaurant){
             console.log(restaurant[0].appSettings.orderTypeOptions)
            this.setState({ cartempty: false,restData:restaurant[0] },()=>{
              if(this.state.restData && this.state.restData.appSettings ){
                let innerArray = this.state.restData.appSettings.orderTypeOptions;
                
                for(let i=0;i<innerArray.length;i++){
                    innerArray[i].isChecked = false;
                  // if(innerArray[i].isChecked){
                  //   this.setState({infoType:innerArray[i].infoType})
                  // }
                }
  ...
  gettime = (flag) => {
    ...
    if (flag === 'min') {
      if (this.state.settings[0]) {
        d.setHours(d.getHours(), d.getMinutes() + Number(this.state.settings[0].settings.time_interval), 0, 0)
      }
    }
    else {
      d.setHours(18, 0, 0, 0)
  ...
  changeCheck(item,index){
    let array= this.state.restData;
    let innerArray = array.appSettings.orderTypeOptions;
    for(let i=0;i<innerArray.length;i++){
      if(innerArray[i].option==innerArray[index].option){
        innerArray[i].isChecked=!innerArray[i].isChecked;
        this.setState({infoType:innerArray[i].infoType})
      }
      else{
        innerArray[i].isChecked=false;
    ...
    array.appSettings.orderTypeOptions=innerArray;
    this.setState({restData:array})
  }
  render() {
    return (
      <Page>
          ...
          <Content >
            <View style={this.state.loader ? { opacity: 0.5 } : null} pointerEvents={this.state.loader ? "none" : "auto"}>
              ...
              <CardTile style={[paddings.p10, margins.mB10]}>
                <View style={{ paddingBottom: 5 }}><Text style={{ color: standardColors.black }}>Billing Details</Text></View>
            ...
            <View style={{ flex: 1, justifyContent: 'center', alignItems: 'center', backgroundColor: 'white' }}>
              <Image style={{ height: 300, width: 300, marginTop: -80 }} source={require('../assets/images/empty6.png')} />
              <Text>Empty Menu</Text>
             <Text style={{ color: 'grey' }}>Looks like you haven't made your choice yet...</Text>
              <TouchableOpacity style={{ backgroundColor: standardColors.appGreenColor, marginVertical: 10,marginTop:10, width: scale(200) }}
                onPress={() => Actions.BottomTabsView()}
              >
                <Text style={{ padding: 10, textAlign: 'center', color: 'white' }}>Back To Menu</Text>
              </TouchableOpacity>
            </View>
          )}
        {(!this.state.cartempty) &&
          <PageFooter style={styles.footerWrapper}>
            <TouchableOpacity onPress={() => {
              ...
              let sendRequest={
                id: this.state.cardList[0].restaurantId
              }
              console.log(sendRequest);
              Actions.Home({categories:sendRequest});
            }}>
            ...
            <View style={styles.contentContainer}>
              ...
              <View style={[{ flexDirection: 'column' }, margins.mT15]}>
                ...
                <View style={{ flexDirection: 'column' }}>
                  <List>
                  { this.state.restData != null && this.state.restData.appSettings &&  this.state.restData.appSettings.orderTypeOptions.map((item,index)=>{
                    return (
                    <View style={{ flexDirection: 'row' }} key={index}>
                    <ListItem style={{ borderBottomColor: 'white', marginTop: 0 }}>
                      <CheckBox color={standardColors.appGreenColor} checked={item.isChecked} onPress={() => this.changeCheck(item,index)}>Pickup</CheckBox>
                      <Text style={{ paddingHorizontal: 10 }}>{item.option}</Text>
                    </ListItem>
                  </View>
                  )
                    })}
              ...
              {this.state.infoType["Time"] && (<TouchableOpacity style={[{ flexDirection: 'column' }, margins.mT15]} onPress={this.showtime}>
              ...
              {this.state.infoType["Text"] && this.state.infoType.Text.map((item,index)=>{
                return (<View style={[{ flexDirection: 'column' }, margins.mT15]}>
